...
for template in cloudformation/vpc-stack.yaml cloudformation/rds-stack.yaml cloudformation/ec2-docker-stack.yaml; do
    step "Validando $template..."
    if aws cloudformation validate-template \
        --template-body "file://$template" \
        --region "$AWS_REGION" > /dev/null 2>&1; then
        success "Template válido: $template"
    else
...
aws cloudformation deploy \
    --template-file cloudformation/ec2-docker-stack.yaml \
