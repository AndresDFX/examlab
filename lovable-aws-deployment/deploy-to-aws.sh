...
if [ ! -f "$ENV_FILE" ]; then
    error ".env no encontrado en $PROJECT_ROOT. Ejecuta primero: cd $PROJECT_ROOT && bash setup.sh"
fi
...
info "Ubicación del script: $SCRIPT_DIR"
...
TEMPLATE_FILE="$SCRIPT_DIR/cloudformation/ec2-docker-stack.yaml"

if [ ! -f "$TEMPLATE_FILE" ]; then
    # Fallback to old template if new one doesn't exist
    TEMPLATE_FILE="$SCRIPT_DIR/cloudformation/ec2-stack.yaml"
    if [ ! -f "$TEMPLATE_FILE" ]; then
        error "CloudFormation template no encontrado en $SCRIPT_DIR/cloudformation/"
    fi
fi
