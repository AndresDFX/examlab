 ...
 * del curso. Se usa para alimentar a la IA con el CONTENIDO real del curso
 * (generación de preguntas Kahoot desde el material — Goal #18).
 *
 * COPIA VERBATIM de `src/modules/contents/material-extract.ts` (Deno no
 * importa de `src/`); idéntica también a la copia en
 * `supabase/functions/tutor-chat/material-extract.ts`. Si cambias una,
 * sincroniza las tres. La parte de unzip (fflate) NO vive acá — la hace el
 * handler del edge con los bytes ya descargados de Storage; acá solo van las
 * transformaciones puras string/JSON → texto.
 */
