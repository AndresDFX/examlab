...
export function identifySystemPrompt(lang: IdentifyLang = "es"): string {
  if (lang === "en") {
    return [
      ...
      "3. statement: a clean, SELF-CONTAINED statement, without the numbering label of the original text. Keep the teacher's wording and intent; fix only obvious typos. Self-contained means it reads without anything else: if the question leans on a case, a shared prompt, a table or a code snippet that lives in ANOTHER question or in a heading above it, copy that context IN FULL into this statement. Never leave references such as 'from the previous item', 'the case described above' or 'using the class from exercise 2': on the platform each question is answered on its own and the order can change.",
  ...
  return [
    ...
    "3. statement: enunciado limpio y AUTOCONTENIDO, sin el rótulo de numeración del texto original. Conservá la redacción y la intención del docente; corregí solo erratas obvias. Autocontenido quiere decir que se entiende sin leer nada más: si la pregunta se apoya en un caso, un enunciado común, una tabla o un fragmento de código que están en OTRA pregunta o en un encabezado de arriba, copiá ese contexto COMPLETO dentro de este enunciado. Nunca dejes referencias como «del punto anterior», «el caso ya descrito» o «usando la clase del ejercicio 2»: en la plataforma cada pregunta se responde sola y el orden puede cambiar.",
