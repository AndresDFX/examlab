-- ══════════════════════════════════════════════════════════════════════
-- Alinear generate_course_acta al algoritmo de notas CANÓNICO del gradebook.
--
-- La revisión funcional (workflow 2026-06-30) confirmó que el acta sellada
-- divergía de lo que ven docente/estudiante en pantalla en TRES puntos:
--
--   G1  Nota FINAL: el acta promediaba las NOTAS DE LOS CORTES (avg-de-cortes);
--       el gradebook y la vista del estudiante promedian PLANO todos los items
--       + asistencias en una sola pasada. Difieren cuando los pesos de items
--       dentro de un corte no suman exactamente el peso del corte (asignación
--       parcial, permitida). → Ahora el acta acumula TODOS los items (de todos
--       los cortes, incluida la asistencia) y llama compute_weighted_grade una
--       sola vez. Las notas POR CORTE se siguen mostrando igual.
--
--   G2  Asistencia: el acta usaba (pct/100)*scale_max, ignorando grade_scale_min;
--       el gradebook usa min + pct*(max-min). En cursos con escala que no
--       empieza en 0, la asistencia (y la final) quedaban subestimadas en el
--       documento legal. → Ahora escala al rango [min,max].
--
--   G6  Múltiples intentos: el LEFT JOIN a submissions emitía UNA FILA POR
--       INTENTO, así que un examen con N intentos entraba N veces al weighted
--       avg (ponderaba ×N) mezclando las notas de cada intento. → Ahora se
--       colapsa a UNA nota efectiva por examen respetando exams.retry_mode
--       (last/average/highest), igual que computeAttemptGrade del gradebook.
--       Solo cuentan intentos finalizados (status completado/sospechoso).
--
-- 'tarde' como presente: el acta YA contaba IN ('presente','tarde') y esa es la
-- decisión de producto (2026-06-30) → la UI se alineó al acta (no al revés), así
-- que acá no cambia. compute_weighted_grade es el mismo helper (null=0 con peso,
-- null-si-ninguno) que computeWeightedGrade en TS.
--
-- El resto de la función (autorización, talleres M:N vía workshop_courses,
-- proyectos, papelera, snapshot/hash, ON CONFLICT regenerar) se copia VERBATIM
-- de 20260978000000.
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    FOR v_cut IN
      ...
      ORDER BY position
    LOOP
      -- Exámenes del corte (1:N al curso). Papelera excluida. G6: colapsa los
      -- intentos a UNA nota efectiva por examen según retry_mode (no una fila
      -- por intento). Solo intentos finalizados.
