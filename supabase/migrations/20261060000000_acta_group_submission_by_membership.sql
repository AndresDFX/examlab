-- ══════════════════════════════════════════════════════════════════════
-- generate_course_acta atribuía la nota de taller/proyecto GRUPAL solo al "último
-- editor" (la fila de submission grupal tiene user_id = ese editor). Para los demás
-- miembros el LEFT JOIN por user_id no matcheaba → score NULL → compute_weighted_grade
-- lo cuenta como 0 con peso completo → nota final del acta (y del certificado) DEPRIMIDA
-- para N-1 miembros del grupo (podía reprobarlos en un documento legal).
--
-- Fix: resolver la submission por MEMBRESÍA (user_id propio O miembro del grupo de la
-- submission), vía LATERAL con LIMIT 1 que PREFIERE la fila grupal (evita doble conteo
-- en modo mixto si el alumno tuviera además una fila individual). Solo cambian los dos
-- JOINs (talleres y proyectos); el resto es idéntico. Migración forward.
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Talleres: resolver la submission por MEMBRESÍA (propia O grupal),
      -- prefiriendo la fila grupal — antes el JOIN por user_id dejaba a los
      -- miembros no-editores con score NULL (= 0 en la nota final).
      ...
      -- Proyectos: idem, por membresía de grupo (project_group_members).
      v_cut_items := v_cut_items || COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'weight', pc.weight,
