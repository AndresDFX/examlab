  ...
  const openCourse = async (c: CourseRow) => {
    ...
    try {
      const [exRes, wsRes, ctRes, enrRes] = await Promise.all([
        ...
        supabase.from("workshops").select("id", { count: "exact", head: true }).eq("course_id", c.id),
        supabase.from("course_teachers").select("user_id").eq("course_id", c.id),
        supabase.from("course_enrollments").select("user_id", { count: "exact", head: true }).eq("course_id", c.id),
      ]);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{courses.length} cursos matriculados · clic para ver detalles</p>
      </div>

      {courses.length === 0 ? (
        <Card><CardContent className="p-10 text-center text-muted-foreground text-sm">No estás matriculado en ningún curso.</CardContent></Card>
      ) : (
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {selected.start_date && selected.end_date && (
                  <Badge variant="secondary" className="text-[10px]">
                    {new Date(selected.start_date + "T00:00").toLocaleDateString()} → {new Date(selected.end_date + "T00:00").toLocaleDateString()}
                  </Badge>
                )}
              </div>
              ...
              <div className="rounded-md border p-3 space-y-2">
                ...
                <div className="grid grid-cols-3 gap-3 text-xs">
                  <div>
                    ...
                    <div className="font-medium tabular-nums">{selected.grade_scale_min} – {selected.grade_scale_max}</div>
                  </div>
                  <div>
                ...
                <>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="rounded-md border p-3 text-center">
                      ...
                      <div className="text-lg font-semibold tabular-nums">{detail.examsCount}</div>
                      <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Exámenes</div>
                    </div>
                    <div className="rounded-md border p-3 text-center">
                      <Hammer className="h-4 w-4 mx-auto text-amber-500 dark:text-amber-400 mb-1" />
                      <div className="text-lg font-semibold tabular-nums">{detail.workshopsCount}</div>
                      <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Talleres</div>
                    </div>
                    <div className="rounded-md border p-3 text-center">
                      <UserCog className="h-4 w-4 mx-auto text-emerald-500 dark:text-emerald-400 mb-1" />
                      <div className="text-lg font-semibold tabular-nums">{detail.studentsCount}</div>
                      <div className="text-[10px] text-muted-foreground uppercase tracking-wide">Estudiantes</div>
                    </div>
                  </div>
                    ...
                    {detail.teachers.length === 0 ? (
                      ...
                      <ul className="space-y-1">
                        {detail.teachers.map((t, i) => (
                          <li key={i} className="text-sm flex justify-between gap-2 border-b last:border-b-0 pb-1">
                            <span className="font-medium truncate">{t.full_name}</span>
                            <span className="text-xs text-muted-foreground truncate">{t.institutional_email}</span>
                          </li>
                        ))}
                      </ul>
                    )}
