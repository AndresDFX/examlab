/**
 * Editor de grupos para un proyecto.
 *
 * Espejo de WorkshopGroupsEditor — drag & drop nativo, modo mixto
...
interface Props {
  projectId: string;
  /** Cursos vinculados al proyecto (incluye el primario). Se usan para
   *  listar matriculados elegibles para grupos. */
  courseIds: string[];
}
          ...
          <div className="space-y-3">
            ...
            {groups.map((g) => {
              ...
              return (
                <Card
                  key={g.id}
                  className={isOver ? "ring-2 ring-primary/60 transition-all" : "transition-all"}
