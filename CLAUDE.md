...
El edge `tutor-chat` ([supabase/functions/tutor-chat/index.ts](supabase/functions/tutor-chat/index.ts)) arma el contexto del Tutor IA. Antes solo pasaba los **títulos** de los contenidos generados (`{{course_content_topics}}`), así que ante preguntas sobre el material el tutor respondía "solo tengo el título". Ahora extrae el **texto real** de los archivos legibles (`md` / `pptx-source` / `txt`) que viven inline en `generated_contents.files[].body` (el módulo de Contenidos guarda ahí el texto al generar/editar), lo concatena por documento (tope 6K/doc, 16K total) y lo inyecta vía `courseMaterial`.

- `buildTutorSystemPrompt` (en [src/modules/tutor/tutor-prompt.ts](src/modules/tutor/tutor-prompt.ts) + la copia Deno) gana un placeholder dedicado `{{course_content_material}}`. Para que el contenido llegue al modelo **sin re-sembrar `ai_prompts`**, cuando el template NO contiene ese placeholder (la fila ya sembrada y los overrides del docente solo conocen `{{course_content_topics}}`) el material se **pliega dentro del bloque de topics**. El `FALLBACK_TEMPLATE` del edge y el `defaultPrompt` del panel admin sí ganaron una sección dedicada "## Contenido del material (extractos)".
