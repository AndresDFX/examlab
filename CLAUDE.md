...
2. **Client-side vía dialog genérico** — encuestas / pizarras / contenidos. Dialog compartido [DuplicateOptionsDialog](src/shared/components/DuplicateOptionsDialog.tsx): recibe `options[{param,label,hint,defaultChecked}]` y devuelve `Record<param, boolean>` en `onConfirm`; la lógica de inserción vive en el caller. Flags:
...
3. **Pre-llenar el form de creación** — banco de preguntas (`duplicate` en [app.teacher.question-bank.tsx](src/routes/app.teacher.question-bank.tsx)). Una pregunta es atómica (sin sub-partes), así que "qué copiar" ES el propio formulario: abre el dialog en modo crear (`editing=null`) pre-llenado con los datos de la fuente; el docente ajusta y guarda la variante (no inserta hasta confirmar).

**Cursos** ya tenían duplicar propio (`doDuplicate` en [app.admin.courses.tsx](src/routes/app.admin.courses.tsx)) con su flag `dupCopyBoard` (copiar tablero/sesiones). **Plantillas de reporte** tienen "Duplicar como privada". Videos y certificados no se duplican (se re-agregan / se emiten).
