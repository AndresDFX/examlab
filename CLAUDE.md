...
| `src/modules/contents/material-extract.ts` ↔ `supabase/functions/tutor-chat/material-extract.ts` (copia Deno — Deno no importa de `src/`)                                                                                                                                                                                                                                                                              | Helpers PUROS de extracción de texto del material del curso: `notebookToReadableText`, `docxXmlToText`, `pptxSlideXmlToText`, `isOfficeDoc`/`isNotebook`/`isReferenceableFile`/`extensionOf`. El unzip (fflate) vive solo en el edge | Divergen → el tutor extrae distinto texto del que el front anticipa (o el picker `#` ofrece archivos que el edge no sabe leer); el alumno ve material incompleto o referencias que el tutor ignora |
